...
const randomWords = ["Happy New Year", "Wishing You Success"];
const wordDotsMap = {};
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 3, "Gabriola, Huawen Amber", "90px");
});
...
const helpContent = {
    shellType: {
        ...
        body: "The type of firework you want to launch. Choosing 'Random' will give you a very enjoyable experience!",
    },
    shellSize: {
        header: "Firework Size",
        body: "The larger the firework, the wider the display area, but larger fireworks also require more device performance. Big fireworks may cause your device to lag.",
    },
    quality: {
        header: "Quality",
        body: "If the animation isn't smooth, try lowering the quality. Higher quality means more sparks from the fireworks, but it might slow down your device.",
    },
    skyLighting: {
        header: "Sky Lighting",
        body: "When fireworks explode, the background will light up. If your screen looks too bright, you can set it to 'Dim' or 'Off'.",
    },
    scaleFactor: {
        header: "Scaling",
        body: "Bring yourself closer to or farther from the fireworks. For larger fireworks, you can choose a smaller scaling value, especially on phones or tablets.",
    },
    wordShell: {
        header: "Text Fireworks",
        body: "When enabled, text-shaped fireworks will appear, such as 'Happy New Year', 'Best Wishes', etc.",
    },
    autoLaunch: {
        header: "Auto Launch Fireworks",
        body: "When enabled, you can sit back and enjoy the fireworks. You can also turn it off and launch fireworks manually by clicking the screen.",
    },
    finaleMode: {
        header: "Launch More Fireworks Simultaneously",
        body: "Automatically launches more fireworks at the same time (requires 'Auto Launch Fireworks' to be enabled first).",
    },
    hideControls: {
        header: "Hide Control Buttons",
        body: "Hides the buttons at the top of the screen. If you want to take screenshots or need a seamless experience, you can hide the buttons. You can still open settings from the top-right corner.",
    },
    fullscreen: {
        header: "Fullscreen",
    ...
    longExposure: {
        ...
        body: "Keeps the sparks left behind by the fireworks.",
    },
};
...
function getWordDots(word) {
	...
	var res = MyMath.literalLattice(word, 3, "Gabriola, Huawen Amber", fontSize + "px");
...
class Shell {
	...
	burst(x, y) {
		...
		if (typeof this.color === "string") {
			...
			throw new Error("Invalid firework color. Expected a string or an array of strings, but got: " + this.color);
...
// If it's the CodePen profile header, no audio is needed; just initialize.
if (IS_HEADER) {
    init();
} else {
    // Allow status to render, then preload assets and start the app.
    setLoadingStatus("Igniting the fuse...");
    setTimeout(() => {
        // Load only the soundManager
        var promises = [soundManager.preload()];

        // Call init after soundManager is loaded
